...
> **Real-time Emergency Relief Platform powered by XRP Ledger & RLUSD**

[![XRP Ledger](https://img.shields.io/badge/XRP_Ledger-Testnet-blue)](https://xrpl.org/)
[![RLUSD](https://img.shields.io/badge/RLUSD-Stablecoin-green)](https://ripple.com/solutions/stablecoins/)
...
**Pulse X** is a transparent, blockchain-based emergency relief donation platform that leverages the XRP Ledger's speed, low fees, and escrow capabilities alongside **RLUSD (Ripple's USD-backed stablecoin)** to ensure rapid, accountable disaster response funding. Built for real-world impact, Pulse X enables donors to contribute in both XRP and RLUSD, providing price stability while maintaining the benefits of blockchain transparency. Relief organizations receive funds efficiently through intelligent allocation mechanisms.
...
- 💰 **RLUSD Stablecoin Integration** - First-class support for Ripple's USD-backed stablecoin (RLUSD) alongside XRP
  - 💵 **Price Stability** - Donate in RLUSD to avoid cryptocurrency volatility
  - 🔄 **Automatic Trustline Setup** - Seamless onboarding with guided trustline configuration
  - 🚰 **Test RLUSD Faucet** - Get test RLUSD instantly for testnet donations
  - 📊 **Dual Balance Tracking** - Monitor both XRP and RLUSD balances in real-time
...
**RLUSD** is Ripple's enterprise-grade USD-backed stablecoin built on the XRP Ledger. Pulse X leverages RLUSD to provide donors with a stable, predictable donation currency while maintaining all the benefits of blockchain transparency and speed.

### Why RLUSD Matters for Emergency Relief

- **💰 Price Stability** - Donations maintain their USD value, crucial for emergency budgeting
- **⚡ Fast Settlement** - Same 3-5 second finality as XRP transactions
- **🔗 Native XRPL Asset** - No bridge protocols or wrapped tokens needed
- **💸 Low Fees** - Fraction-of-a-cent transaction costs
- **🏦 Regulatory Clarity** - Enterprise-grade stablecoin with compliance built-in
- **🌍 Global Accessibility** - No traditional banking infrastructure required

### RLUSD Features in Pulse X

#### 1. **Trustline Management**
```http
POST /api/rlusd/trustline
```
Automatically generate unsigned trustline transactions for users to sign with their XRPL wallet. Trustlines are required once per account to hold RLUSD.

#### 2. **Test RLUSD Faucet**
```http
POST /api/rlusd/faucet
```
Instantly receive 500 test RLUSD for development and testing purposes.

#### 3. **Balance Checking**
```http
GET /api/rlusd/balance/{address}
```
Query real-time RLUSD balances for any XRPL address.

#### 4. **Dual-Currency Donations**
Donors can choose their preferred currency:
- **XRP** - Native XRPL asset, volatile but highly liquid
- **RLUSD** - Stable value, predictable impact

#### 5. **Organization Escrows**
RLUSD escrows work identically to XRP escrows, providing:
...
- RLUSD Issuer account (automatically generated by setup script)
...
# Setup RLUSD issuer and configure trustlines
cd app
python3 ../backend/scripts/setup_rlusd.py
...
│   │   │   ├── donations.py           # Donation endpoints (XRP & RLUSD)
...
│   │   │   └── donation_tracking.py   # Track donation flow
...
│   ├── setup_rlusd.py                 # 🌟 Setup RLUSD issuer & configure trustlines
...
- **Currency-aware**: Handles both XRP and RLUSD in the same batch

### Multi-Currency Architecture

Pulse X natively supports both XRP and RLUSD through a unified architecture:

#### XRP Flow
1. Donor sends XRP → Platform Pool Wallet
2. Batched into escrows → Released to organizations
3. Subject to price volatility but highly liquid

#### RLUSD Flow
1. Donor establishes trustline (one-time setup)
2. Donor sends RLUSD → Platform Pool Wallet
3. Batched into escrows → Released to organizations
4. Price-stable, 1 RLUSD ≈ 1 USD

**Smart Selection**: The platform tracks both currencies separately and allows organizations to specify their preferred currency for receiving funds, enabling optimal liquidity management.
...
```http
POST /donations
Body: { donor_address, amount_xrp, amount_rlusd, currency: "XRP" | "RLUSD" }
```

Create a new donation in XRP or RLUSD

```http
GET /donations/{donor_address}
```

Retrieve donation history for a donor (includes both XRP and RLUSD donations)
...
Generate unsigned trustline transaction for RLUSD. User signs with their wallet.

```http
POST /api/rlusd/faucet
Body: { address }
Response: { status, tx_hash, amount: 500 }
```

Request 500 test RLUSD from the faucet (testnet only)

```http
GET /api/rlusd/balance/{address}
Response: { address, balance, configured }
```

Check RLUSD balance for any XRPL address
...
2. **Setup RLUSD (Optional)** - Create trustline if donating in RLUSD stablecoin
3. **Get Test Funds** - Use faucets to receive test XRP and RLUSD
4. **Browse Emergencies** - View active disasters needing support
5. **Choose Currency** - Select XRP (fast, volatile) or RLUSD (stable, predictable)
6. **Donate** - Contribute in any amount
7. **Track Impact** - Watch real-time dashboard showing fund allocation
8. **Verify on Ledger** - Click through to see your transaction on XRPL explorer
...
- 💵 **RLUSD issuer security**: Issuer wallet requires highest security standards; use hardware wallets for mainnet
- 🔐 **Trustline management**: Users retain full control over their RLUSD trustlines
...
- **Ripple** for the XRP Ledger, RLUSD stablecoin, and comprehensive developer resources
- **FastAPI** community for excellent documentation
- **React** team for the amazing frontend framework
- **XRPL Foundation** for supporting blockchain innovation
...
- [ ] **Enhanced RLUSD Features**
  - [ ] Automatic XRP ↔ RLUSD conversion at donation time
  - [ ] RLUSD yield optimization for pooled funds
  - [ ] Multi-stablecoin support (USDC, USDT on XRPL)
- [ ] Mobile app (React Native) with built-in XRPL wallet
...
- [ ] NFT receipts for donations (on XRPL)
- [ ] Cross-chain bridge for Ethereum-based donations
